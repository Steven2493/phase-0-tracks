...
#4. method that will iterate over array or hash to return false when length is than the 
hashCities = {texas: "austin", illinois: "chicago", california: "sacramento"}
hashCities.keep_if {|state,city| state.length >= 8}

#false when city.length is <= 6 length
arrayCity = ["Austin","New York","Google"]
arrayCity.drop_while {|city| city.length <= 6  }
